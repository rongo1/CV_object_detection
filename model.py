...
class CNNBlock(nn.Module):
    def __init__(self, in_channels, out_channels, **kwargs):
        ...
        self.conv = nn.Conv2d(in_channels, out_channels, bias = False, kernel_size = 1, **kwargs)
...
class Yolov1(nn.Modelu):
    def __init__(self, in_channels = 3, **kwards):
    ...
    def forward(self, x):
        x = self.darknet(x)
        return self.fcs(torch.flatter(x, start_dim = 1))
    def _create_conv_layers(self, architecture):
        layers = []
        in_channels = self.in_channels
        for x in self.architecture:
            if type(x) == tuple:
                layers += [self.CNNBlock(in_channels, x[1], kernel_size = x[0], stride = x[2], padding = x[3])]
                in_channels = x[1]
            elif type(x) == str:
